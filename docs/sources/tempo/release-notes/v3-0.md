...
The [`tempo-cli redact`](/docs/tempo/<TEMPO_VERSION>/operations/tempo_cli/#redact-traces) command submits redaction jobs to the [backend scheduler](/docs/tempo/<TEMPO_VERSION>/reference-tempo-architecture/components/compaction/#backend-scheduler), which rewrites the affected blocks in object storage.
Use `--dry-run` to preview match counts without rewriting blocks; dry-run jobs don't block tenant compaction or retention.
You can monitor job progress through the [`/status/backendscheduler`](/docs/tempo/<TEMPO_VERSION>/api_docs/#backend-scheduler-job-status) endpoint. [[PR 6832](https://github.com/grafana/tempo/pull/6832)] [[PR 7700](https://github.com/grafana/tempo/pull/7700)]
