...
When span pruning is enabled for the request, the response may include a `status` field set to `PARTIAL` and a `message` field that explains the pruning outcome:
