...
Tempo keeps memcached client connection pools warm by default so query bursts don't trigger a dial storm at the start of each surge.
The default `max_idle_conns` is 100 per memcached server, and idle connections aren't closed unless you set `min_idle_conns_headroom_percentage` to a positive value.
