...
- `--q <TraceQL filter>` TraceQL spanset filter that returns only matching spans (v2 API only). For example, `--q '{ span.http.status_code = 500 }'`.
- `--keep-hierarchy` Include the ancestor path from the root to each matched span. Requires `--q`.
...
Example filtering spans with a TraceQL filter:

```bash
tempo-cli query api trace-id http://tempo:3200 f1cfe82a8eef933b --q '{ span.http.status_code = 500 }'
```

Example including ancestor spans for a filtered result:

```bash
tempo-cli query api trace-id http://tempo:3200 f1cfe82a8eef933b --q '{ span.http.status_code = 500 }' --keep-hierarchy
```
...
You can select traces by explicit trace ID or with a TraceQL query.
...
- `--dry-run` Evaluate the query and report match counts without rewriting any blocks.
...
Dry-run a query to see how many traces match before rewriting blocks:
