...
Use `--dry-run` to preview how many traces match without rewriting any blocks.
...
- `--dry-run` Evaluate the selector and report match counts without rewriting any blocks. Dry-run jobs don't block tenant compaction, retention, or rescan. Apply-mode redaction (the default) is an exclusive tenant operation that blocks compaction and retention until the batch completes and its quiescence window ends.
